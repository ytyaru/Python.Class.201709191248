...
class SPN(metaclass=OctaveClass, Min=-1, Names={'ja': '国際式'}, Descriptions={'ja': '88鍵盤の最低音がA0になる。'}):
    def __init_subclass__(cls, **kwargs): super().__init_subclass__(**kwargs)
class YAMAHA(metaclass=OctaveClass, Min=-2, Names={'ja': 'YAMAHA式'}, Descriptions={'ja': '根拠不明。'}):
    def __init_subclass__(cls, **kwargs): super().__init_subclass__(**kwargs)
class ZERO(metaclass=OctaveClass, Min=0, Names={'ja': 'ゼロ式'}, Descriptions={'ja': 'OctaveClassと同値。最低周波数のオクターブを0とする。'}):
    def __init_subclass__(cls, **kwargs): super().__init_subclass__(**kwargs)
