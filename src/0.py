...
class OctaveClass(metaclass=ConstMeta):
    """
    def __new__(cls, _min, names, descriptions):
        Min = _min
        Names = names
    ...
    def __init_subclass__(cls, *args):
        print('__init_subclass__', cls, args)
        Min = args[0]
        Names = args[1]
        Descriptions = args[2]
    """
...
"""
"""
class SPN(OctaveClass):
    def __new__(cls):
#        super().__new__(-1, {'ja': '国際式'}, {'ja': '88鍵盤の最低音がA0になる。'})
        super().__init_subclass__(-1, {'ja': '国際式'}, {'ja': '88鍵盤の最低音がA0になる。'})
class YAMAHA(OctaveClass):
    def __new__(cls):
#        super().__new__(-2, {'ja': 'YAMAHA式'}, {'ja': '根拠不明。'})
        super().__init_subclass__(-2, {'ja': 'YAMAHA式'}, {'ja': '根拠不明。'})
class ZERO(OctaveClass):
    def __new__(cls):
#        super().__new__(0, {'ja': '国際式'}, {'ja': 'OctaveClassと同値。最低周波数のオクターブを0とする。'})
        super().__init_subclass__(0, {'ja': '国際式'}, {'ja': 'OctaveClassと同値。最低周波数のオクターブを0とする。'})
"""
