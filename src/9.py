...
#    def __init__(self): pass
    ...
    def __setattr__(self, name, value):
        print('aaaaaaaaa')
#        if name in getattr(self, name): raise self.ConstError('readonly。再代入禁止です。')
#        if name in self.__getattr__(name): raise self.ConstError('readonly。再代入禁止です。')
        if name in self.__dict__.keys(): raise self.ConstError('readonly。再代入禁止です。')
        super(ConstMeta, self).__setattr__(name, value)

    def __init_subclass__(cls, Min, Names, Descriptions, **kwargs):
...
for cd in class_datas:
    Type = type(cd['name'], (object,), cd['members'])
